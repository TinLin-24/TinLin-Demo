...
- (void)prepareLayout {
    ...
    for (int section= 0; section<sectionCount; section++) {
        ...
        for (int row=0; row<itemCount; row++) {
            ...
            BOOL isNeed = NO;
            if (delegate && [delegate respondsToSelector:@selector(collectionView:layout:shouldCustomLayoutForItemInSection:)]) {
                isNeed = [delegate collectionView:self.collectionView layout:self shouldCustomLayoutForItemInSection:section];
            }
            if (isNeed) {
